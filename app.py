    ...
    for root, dirs, files in os.walk(tmp_dir):
        ...
        if "02_stt" in root:
            ...
            if seg_files:
                ...
                video_path = (
                    os.path.join(project_path, rel_path, video_name)
                    if rel_path
                    else os.path.join(project_path, video_name)
                )

                # Find the actual video file (try common extensions)
                for ext in [".mp4", ".mkv", ".avi", ".mov", ".webm"]:
                    full_video_path = (
                        video_path.replace(os.path.splitext(video_path)[1], "") + ext
                        if "." in video_path
                        else video_path + ext
                    )
...
def display_voice_refinement_modal():
    ...
    if not openai_token:
        st.error(
            "OpenAI API token is not configured. Please set it in the OpenAI Settings."
        )
    ...
    if not videos_with_subs:
        ...
        st.info(
            "Add subtitle files (.srt) or run the redub pipeline to generate segments."
        )
        if st.button("Close", use_container_width=True):
            st.session_state.show_voice_refinement_modal = False
            st.rerun()
    ...
    if "vr_instructions" not in st.session_state:
        st.session_state.vr_instructions = current_voice_settings.get(
            "voice_instructions", ""
        )
    ...
    selected_video = next(
        (v for v in videos_with_subs if v["path"] == selected_video_path), None
    )
    ...
    if segment_source == "srt":
        st.info(
            f"📄 Using external subtitle: {os.path.basename(selected_video['subtitle_files'][0])}"
        )
    else:
    ...
    with col2:
        if segment_index < len(segments):
            ...
            st.text(
                f"Time: {seg.start:.1f}s - {seg.end:.1f}s ({seg.end - seg.start:.1f}s)"
            )
            st.text(
                f"Text: {seg.text[:100]}..."
                if len(seg.text) > 100
    ...
    selected_voice = st.selectbox(
        ...
        index=list(voice_options.keys()).index(st.session_state.vr_voice)
        if st.session_state.vr_voice in voice_options
        else 0,
    )
    ...
    with col1:
        if st.button(
            ...
            help="Analyze the segment audio with AI",
        ):
            with st.spinner("Extracting audio sample..."):
                try:
                    ...
                    samples_dir = os.path.join(
                        "redubber_tmp", rel_path, "voice_samples"
                    )
                    os.makedirs(samples_dir, exist_ok=True)

                    original_sample_path = os.path.join(
                        samples_dir, "original_sample.m4a"
                    )

                    # Extract audio sample
                    extract_audio_sample(
                        selected_video_path, seg.start, seg.end, original_sample_path
                    )
                    ...
                    with st.spinner("Analyzing voice with AI..."):
                        analysis = analyze_voice_with_gpt4o(
                            original_sample_path, openai_token
                        )
                        st.session_state.vr_voice = analysis["recommended_voice"]
                        st.session_state.vr_instructions = analysis[
                            "voice_instructions"
                        ]
    ...
    with col2:
        ...
        if st.button(
            ...
            help="Generate TTS sample for comparison"
            if can_preview
            else "Run Analyze first",
        ):
            with st.spinner("Generating TTS sample..."):
                try:
                    ...
                    generated_sample_path = os.path.join(
                        samples_dir, "generated_sample.m4a"
                    )
    ...
    with col1:
        ...
        if st.session_state.vr_original_sample and os.path.exists(
            st.session_state.vr_original_sample
        ):
    ...
    with col2:
        ...
        if st.session_state.vr_generated_sample and os.path.exists(
            st.session_state.vr_generated_sample
        ):
...
def main():
    ...
    if "openai_config_loaded" not in st.session_state:
        ...
        st.session_state.target_language = (
            saved_target_language if saved_target_language else "eng"
        )
    ...
    with st.sidebar:
        ...
        with st.expander("🤖 OpenAI Settings", expanded=False):
            ...
            selected_default_voice = st.selectbox(
                ...
                index=list(default_voice_options.keys()).index(current_default_voice)
                if current_default_voice in default_voice_options
                else 4,  # nova is index 4
        ...
        for project in existing_projects:
            ...
            button_type = (
                "primary"
                if st.session_state.current_page == project_path
                else "secondary"
            )
    ...
    with st.spinner("Scanning project folder..."):
        ...
        for video_file in video_files:
            ...
            db_manager.add_video_file(
                project_id, str(video_file), video_file.name, video_language
            )
        ...
        for subtitle_file in subtitle_files:
            ...
            db_manager.add_subtitle_file(
                project_id, str(subtitle_file), subtitle_file.name, subtitle_language
            )
        ...
        with st.container():
            ...
            with col3:
                ...
                subtitle_files = db_manager.get_subtitle_files_for_video(
                    project_id, video_file["filename"]
                )
                ...
                if subtitle_files:
                    for sub_file in subtitle_files:
                        if sub_file["language"]:
                            st.markdown(
                                f"{get_language_flag(sub_file['language'])} `{sub_file['language']}`"
                            )
                        else:
                            st.markdown("❓ `unknown`")
                else:
