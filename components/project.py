...
def get_project_voice_settings():
    ...
    logging.info(
        f"get_project_voice_settings: db_manager={db_manager is not None}, project_path={project_path}"
    )
...
def run_redub_pipeline(video_data: dict, progress_callback=None):
    ...
    if not openai_token:
        raise ValueError(
            "OpenAI token is required. Please configure it in the sidebar."
        )
    ...
    if project_data:
        source_language_override = db_manager.get_source_language_override(
            project_data["id"]
        )
...
def run_pipeline_stage(
    video_data: dict, stage: str, progress_callback=None, stage_complete_callback=None
):
    """
    ...
    if stage == "audio":
        ...
        redubber.extract_audio_chunks(
            reproj, progress_callback=make_progress_callback(stage_name)
        )
        if progress_callback:
        ...
        redubber.get_text_and_segments(
            reproj, compact=True, progress_callback=make_progress_callback(stage_name)
        )
        if progress_callback:
            progress_callback(stage_name, 100)
        ...
        redubber.tts_segments(
            reproj, segments, progress_callback=make_progress_callback(stage_name)
        )
        if progress_callback:
            progress_callback(stage_name, 100)
        ...
        # Assemble audio
        duration = redubber.get_media_duration(video_path)
        redubber.assemble_long_audio(
            segments,
            reproj,
            duration,
            progress_callback=make_progress_callback(stage_name),
        )
        if progress_callback:
            progress_callback(stage_name, 100)
...
def check_video_readiness(video_data, target_language):
    ...
    has_target_subtitle = (
        target_language in subtitle_languages or has_matching_external_subtitle
    )
...
def display_current_project_page():
    ...
    if not project_data:
        ...
        project_id = db_manager.add_project(
            st.session_state.current_project_path, project_name
        )
    ...
    if not has_scan_data:
        st.info(
            "🔄 Project needs to be initialized. Click 'Initialize Project' to scan files and analyze content."
        )
    ...
    with col1:
        if not has_scan_data:
            if st.button(
                "🚀 Initialize Project", use_container_width=True, type="primary"
            ):
    ...
    with st.expander("Project Settings", expanded=False):
        ...
        selected_override = st.selectbox(
            ...
            index=list(source_language_options.keys()).index(current_override)
            if current_override in source_language_options
            else 0,
        ...
        if (
            ...
            or voice_instructions != current_instructions
        ):
            db_manager.set_voice_settings(
                project_id, selected_voice, voice_instructions
            )
        ...
        if st.button(
            ...
            help="Analyze a video sample to find the best matching voice"
            if openai_token
            else "Configure OpenAI token first",
        ):
...
def initialize_project(project_id: int):
    ...
    try:
        ...
        analysis_results = analyze_project_files(
            project_path, update_progress, target_language
        )
...
def display_project_analysis(project_id: int):
    ...
    if st.session_state.get("autoredub_video"):
        ...
        try:
            ...
            def update_progress(message, progress=None):
                if progress is not None:
                    progress_placeholder.progress(
                        progress / 100, text=f"{message}: {progress}%"
                    )
                else:
            ...
            # Run the redub
            output_file = run_smart_redub(
                video_data, pipeline_status, update_progress, on_stage_complete
            )
    ...
    if analysis_data.get("videos"):
                ...
                if isinstance(subtitles, list):
                    for sub in subtitles:
                        if isinstance(sub, dict):
                            ...
                            if lang and lang != "unknown":
                                sub_langs.append(
                                    f"{prefix}{get_language_flag(lang)} {lang}"
                                )
                            else:
                ...
                table_data.append(
                    {
                        ...
                        "Audio": str(", ".join(audio_langs))
                        if audio_langs
                        else "❌ No audio",
                        "Subtitles": str(", ".join(sub_langs))
                        if sub_langs
                        else "❌ None",
        ...
        effective_filename_filter = (
            filename_filter
            or st.session_state.get("_filename_filter_preserved", "")
            or initial_filename_filter
        )
        effective_hide_completed = (
            show_only_incomplete
            or st.session_state.get("_hide_completed_preserved", False)
            or initial_hide_completed
        )
        effective_debug_mode = (
            debug_mode
            or st.session_state.get("_debug_mode_preserved", False)
            or initial_debug_mode
        )
        ...
        for i, row in enumerate(table_data):
            ...
            if (
                effective_filename_filter
                and effective_filename_filter.lower() not in row["File"].lower()
            ):
                continue
        ...
        if filtered_table_data:
            ...
            if effective_debug_mode:
                col1, col2, col3, col4, col5, col6, col7, col8, col9 = st.columns(
                    [2.3, 0.6, 0.6, 1.1, 1.1, 1.4, 0.5, 0.5, 0.5]
                )
                with col1:
                    st.write("**📁 File**")
                with col2:
            ...
            for display_index, row in enumerate(filtered_table_data):
                ...
                if row.get("IsReady", False):
                        ...
                        with col6:
                            ...
                            if pipeline:
                                display_pipeline_status(
                                    pipeline, f"ready_{display_index}"
                                )
                            else:
                                st.write("—")
                        if effective_debug_mode:
                            with col7:
                                if st.button(
                                    "▶️",
                                    key=f"play_filtered_{display_index}",
                                    help="Play video",
                                ):
                                    st.session_state.video_to_play = analysis_data[
                                        "videos"
                            ...
                            with col8:
                                if st.button(
                                    "🎬",
                                    key=f"redub_{display_index}",
                                    help="Redub video",
                                ):
                                    st.session_state.video_to_redub = analysis_data[
                                        "videos"
                                    ][original_video_index]
                                    st.session_state.show_redub_modal = True
                                    st.rerun()
                            with col9:
                                video_path = analysis_data["videos"][
                                    original_video_index
                                ].get("path", "")
                                pipeline = row.get("Pipeline")
                                ...
                                if st.button(
                                    "⚡",
                                    key=f"autoredub_{display_index}",
                                    help="Auto-redub without modal",
                                    disabled=is_complete,
                                ):
                                    st.session_state.autoredub_video = analysis_data[
                                        "videos"
                                    ][original_video_index]
                    ...
                    if effective_debug_mode:
                        col1, col2, col3, col4, col5, col6, col7, col8, col9 = (
                            st.columns([2.3, 0.6, 0.6, 1.1, 1.1, 1.4, 0.5, 0.5, 0.5])
                        )
                    else:
                        col1, col2, col3, col4, col5, col6, col7 = st.columns(
                            [2.5, 0.7, 0.7, 1.2, 1.2, 1.5, 0.6]
                    ...
                    if effective_debug_mode:
                        with col7:
                            if st.button(
                                "▶️",
                                key=f"play_normal_{display_index}",
                                help="Play video",
                            ):
                                st.session_state.video_to_play = analysis_data[
                                    "videos"
                        ...
                        with col8:
                            if st.button(
                                "🎬",
                                key=f"redub_normal_{display_index}",
                                help="Redub video",
                            ):
                                st.session_state.video_to_redub = analysis_data[
                                    "videos"
                        ...
                        with col9:
                            ...
                            if st.button(
                                "⚡",
                                key=f"autoredub_normal_{display_index}",
                                help="Auto-redub without modal",
                                disabled=is_complete,
                            ):
                                st.session_state.autoredub_video = analysis_data[
            ...
            if table_data:
                ...
                if analysis_data["videos"]:
                    ...
                    for video in analysis_data["videos"][1:]:
                        ...
                        common_audio_langs = common_audio_langs.intersection(
                            video_audio_langs
                        )
                ...
                if analysis_data["videos"]:
                    ...
                    for video in analysis_data["videos"][1:]:
                        ...
                        common_sub_langs = common_sub_langs.intersection(
                            video_sub_langs
                        )
                ...
                with col4:
                    common_audio_display = (
                        ", ".join(
                            [
                                f"{get_language_flag(lang)}"
                                for lang in sorted(common_audio_langs)
                            ]
                        )
                        if common_audio_langs
                        else "—"
                    )
                ...
                with col5:
                    common_sub_display = (
                        ", ".join(
                            [
                                f"{get_language_flag(lang)}"
                                for lang in sorted(common_sub_langs)
                            ]
                        )
                        if common_sub_langs
                        else "—"
    ...
    if analysis_data["subtitles"]:
        ...
        for sub in analysis_data["subtitles"]:
            standalone_data.append(
                {
                    "File": sub["filename"],
                    "Language": f"{get_language_flag(sub['language'])} {sub['language']}"
                    if sub["language"]
                    else "❓ Unknown",
                }
            )
...
def display_delete_confirmation_dialog():
    ...
    st.warning(
        "This will remove the project from your project list. The actual files on disk will not be deleted."
    )
...
def display_video_modal():
    ...
    if subtitles:
        ...
        for i, subtitle in enumerate(subtitles):
            ...
            with st.expander(
                f"📄 {sub_filename} ({get_language_flag(sub_language)} {sub_language})"
            ):
                try:
...
def display_redub_modal():
    ...
    if st.session_state.get("stage_running", False):
        ...
        def update_stage_progress(stage_label: str, progress: int):
            progress_placeholder.progress(
                progress / 100, text=f"{stage_label}: {progress}%"
            )
        ...
        if stage_to_run:
            try:
                ...
                st.success(
                    f"✅ {result['stage_name']} completed in {format_duration(result['duration_seconds'])}"
                )
                time.sleep(0.5)
                st.rerun()
            except Exception as e:
    ...
    if st.session_state.get("redub_running", False):
        ...
        with st.status("Running redub pipeline...", expanded=True) as status:
            ...
            try:
                ...
                def on_stage_complete(stage_label: str, duration: float):
                    st.toast(
                        f"✅ {stage_label} completed in {format_duration(duration)}"
                    )
    ...
    for stage_id, stage_name, stage_desc in stages:
        ...
        with col2:
            if status == "skipped":
                st.caption("Skipped (external subs available)")
            elif status == "done":
                ...
                if stage_id == "audio":
                    ...
                    st.caption("Done - subtitles generated")
                elif stage_id == "tts":
                    st.caption(f"Done - {pipeline_status.tts_segments} segments")
                elif stage_id == "assemble":
                    st.caption(
                        f"Done - {pipeline_status.target_audio_chunks} audio files"
                    )
                elif stage_id == "final":
                    st.caption("Done - final file ready")
            else:
    ...
    if pipeline_status.is_complete:
        st.success("✅ Video redubbed successfully!")
...
def run_smart_redub(
    ...
    log.info(
        f"run_smart_redub: pipeline_status.has_tts={pipeline_status.has_tts}, has_external_subs={pipeline_status.has_external_subs}"
    )
    ...
    if pipeline_status.has_external_subs:
        ...
        if not pipeline_status.has_tts:
            ...
            log.info(
                f"run_smart_redub: Starting TTS generation for {len(segments)} segments with voice={redubber.voice}"
            )
            ...
            log.info(
                f"run_smart_redub: TTS generation complete in {stage_duration:.1f}s"
            )
            if stage_complete_callback:
                stage_complete_callback(stage_name, stage_duration)
        else:
            log.info("run_smart_redub: Skipping TTS (already done)")
            if progress_callback:
        ...
        assemble_callback(0)
        duration = redubber.get_media_duration(video_path)
        audio_file = redubber.assemble_long_audio(
            segments, reproj, duration, progress_callback=assemble_callback
        )
        stage_duration = time_module.time() - stage_start
        if stage_complete_callback:
            stage_complete_callback(stage_name, stage_duration)
    ...
    segments = redubber.get_text_and_segments(
        reproj, compact=True, progress_callback=stt_callback
    )
    stage_duration = time_module.time() - stage_start
    if stage_complete_callback:
        stage_complete_callback(stage_name, stage_duration)
