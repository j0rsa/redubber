...
class FileScanner:
    ...
    def scan_folder(self, folder_path: str) -> Tuple[List[Path], List[Path]]:
        ...
        # Scan recursively, skipping hidden working directories (.redubber, redubber_tmp, etc.)
        _SKIP_DIRS = {".redubber", "redubber_tmp", "redubber_backups", ".DS_Store"}

        for file_path in folder.rglob("*"):
            # Skip any path that passes through a working/hidden directory
            if any(part in _SKIP_DIRS or part.startswith(".redubber") for part in file_path.parts):
                continue
